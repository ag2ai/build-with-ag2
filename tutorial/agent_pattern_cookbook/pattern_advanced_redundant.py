...
def record_agent_response(
    ...
    taskmaster_agent.context_variables.set(
        context_var_key, recipient.chat_messages[sender][-1]["content"]
    )

    # Increment the approach counter
    taskmaster_agent.context_variables.set(
        "approach_count", taskmaster_agent.context_variables.get("approach_count") + 1
    )

    # Track if we now have all results
    task_completed = all(
        taskmaster_agent.context_variables.get(f"{key}_result") is not None
    ...
    [
        ...
        OnContextCondition(
            target=NestedChatTarget(
                nested_chat_config={"chat_queue": redundant_agent_queue}
            ),
        ...
        OnContextCondition(
            ...
            condition=ExpressionContextCondition(
                ContextExpression("${evaluation_complete} == False")
            ),
            available=ExpressionAvailableCondition(
                ContextExpression("${task_completed} == True")
            ),
        ),
    ]
)
